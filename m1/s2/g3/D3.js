...
for ( let i=0; i< starWarsCharacters.length;i++){
  ...
  switch(carattere.eye_color){
    case 'blue': eyeColor.blue.push(carattere)
                break
    case 'yellow': eyeColor.yellow.push(carattere)
                  break
    case 'brown': eyeColor.brown.push(carattere)
                  break
    case 'red': eyeColor.red.push(carattere)
                break
    case 'blue-gray':eyeColor['blue-gray'].push(carattere)
                break
...
console.log('il peso totale è ' + crewMass)
...
if (crewMass > 1000) {console.log("DANGER! OVERLOAD ALERT : escape from ship now!")}
                    ...
                    else(console.log("Ship is Empty"))

/* ESERCIZIO 8
  Usa un for loop per cambiare il valore della proprietà "gender" di 
  alcuni personaggi dal valore "n/a" a "robot" (Tip: puoi effettuare 
...
/* --EXTRA-- ESERCIZIO 9
  ...
  rimuovi dall'array "charactersNames" le stringhe corrispondenti a
  personaggi con lo stesso nome.
  Una volta fatto crea un console.log per controllare la proprietà 
  length di "charactersNames" prima e dopo l'operazione.
*/
