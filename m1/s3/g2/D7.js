...
*/
  ...
  function insieme(arr1,arr2)  { 
    ...
    const primo = arr1.slice(0,2);
    const ultimo = arr2.slice(-3);
    const insieme = primo + ultimo;
    const maiuscolo = insieme.toUpperCase();
    console.log(maiuscolo);

  }
insieme("epicode","developer")


/* ESERCIZIO 2 (for)
  Scrivi una funzione che torni un array di 10 elementi; 
  ognuno di essi deve essere un valore
   random compreso tra 0 e 100 (incluso).
*/
function random(){
const array = [];
for (let i=0; i<10 ;i++){
  let gen = Math.floor(Math.random() * 101);
  array.push(gen);
}
console.log(array);
}
...
function somma(){
  let numeroFinale= 0; 
numeri.forEach(numero => numeroFinale += numero );
...
*/
function somma2(){
  const tot=numeri.reduce(((temp,curr)=>temp+curr),0);
  console.log(tot);
}
somma2()
...
*/
function arrayinc (n){
  const newArr = numeri.map((numero) =>numero+ n);
  console.log(newArr);
}
arrayinc(3)
...
*/
const arraystringhe = ["EPICODE", "is", "great","school"]
function strings(){
  const newArray =arraystringhe.map((stringa)=>stringa.length);
  console.log(newArray);
}
strings()

/* ESERCIZIO 8 (forEach o for)
  Scrivi una funzione per creare un array contenente tutti 
  i valori DISPARI da 1 a 99.
*/
...
/* ESERCIZIO 10
  Scrivi una funzione per ottenere il numero di film contenuti
   nell'array fornito.
*/

function totaleMovie(){
  const totale= movies.length;
  console.log("il numero di film è " + totale);
}
totaleMovie()
...
function Title(){
  const arrTitle = movies.map((movie) => movie.Title);
...
function millenians (){
  const mille = movies.filter(movie => movie.Year >= 2000)
  console.log(mille);
}
millenians()
